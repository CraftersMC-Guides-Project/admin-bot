...
AUTHORIZED_UNLOCK_USERS = [78397238588276740, 947868777528307742, 845690488752832522, 992284684425904168]  # Add allowed user IDs here
...
development = False
owner_id = 947868777528307742
required_role_id = 1320729567652085780
required_helper_id = 1361716492479107132
...
async def global_command_check(ctx):
    ...
    return required_role_id in role_ids or required_helper_id in role_ids


@bot.event
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        if development and ctx.author.id != owner_id:
            await ctx.send("🛠 The bot is currently in development mode. Only the developer can use it.")
        else:
            await ctx.send("🚫 You are not a member of the Management team.")
    else:
        raise error
...
async def on_ready():
    ...
    activity = discord.Activity(type=discord.ActivityType.watching, name="Market | .help")
