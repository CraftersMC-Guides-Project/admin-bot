...
TOKEN = 'MTM2MDY1NjY1NDMxNTY4ODA5Nw.GiUBzc.aNMjnCdm2oP0pQQWVOholH-j7oMOQHhrKNeQhE'
...
async def showpetchanges(ctx):
    ...
    if len(diff) > 1900:  # Lower threshold since we're showing more data
        file = discord.File(io.StringIO(diff), filename="pet_price_changes.txt")
...
async def updateminion(ctx, minion_id: int, tier: str, price_type: str, price: int):
...
async def showminionchanges(ctx):
    ...
    if len(diff) > 1900:
        file = discord.File(io.StringIO(diff), filename="minion_changes.txt")
...
async def update_minion_market_entry(ctx, args):
    parts = args.split()
    nature = demand = None
...
async def updateminionmarket(ctx, *, args):
...
class MinionMarketChangeView(discord.ui.View):
    ...
    async def commit_changes(self, interaction: discord.Interaction, button: discord.ui.Button):
        if minion_market_temp != minion_market_original:
            ...
            await interaction.response.send_message("✅ Minion changes committed and temporary edits cleared.")
        else:
            await interaction.response.send_message("ℹ️ No Minion changes to commit.")

    @discord.ui.button(label="Undo Last", style=discord.ButtonStyle.danger)
    async def undo_last(self, interaction: discord.Interaction, button: discord.ui.Button):
        if minion_market_history:
            minion_market_temp[:] = minion_market_history.pop()
            await interaction.response.send_message("↩️ Last minion change undone.")
        else:
            await interaction.response.send_message("ℹ️ No minion changes to undo.")

    @discord.ui.button(label="Undo All", style=discord.ButtonStyle.danger)
    async def undo_all(self, interaction: discord.Interaction, button: discord.ui.Button):
        minion_market_temp[:] = load_minion_market_data()
        minion_market_history.clear()
        await interaction.response.send_message("🔄 All minion changes undone.")
...
async def showminionmarketchanges(ctx):
    ...
    if len(diff) > 1900:
        file = discord.File(io.StringIO(diff), filename="minions_diff.txt")
