...
async def help(ctx):
    ...
    embed.add_field(
        name="🔁 `.sync`",
        value="Downloads `mprices.txt` `market.txt` `pets.txt`, `pet-prices.txt` and `minion-prices.txt` from github to sync the bot with latest data.",
        inline=False
    )
...
async def sync(ctx):
    ...
    try:
        ...
        await ctx.send("✅ Synced `mprices.txt`, `market.txt`, `pets.txt`, `pet-prices.txt` and `minion-prices.txt` from GitHub.")
    except Exception as e:
